...
@app.route("/")
def home():
    return jsonify({"status": "CineAI API running 🚀"})
...
def recommend():
    """
    Collaborative-filtering + model-predicted-rating hybrid.
    ...
    # ── 2. Movies the target user has NOT rated ────────────────────────────────
    rated_by_user  = set(ratings[ratings["user_id"] == user_id]["movie_id"])
    candidate_ids  = (
        set(ratings[ratings["user_id"].isin(top_users)]["movie_id"])
        - rated_by_user
    )

    if not candidate_ids:
        # Fallback: top-rated movies globally the user hasn't seen
        candidate_ids = set(movie_avg.nlargest(200).index) - rated_by_user

    # ── 3. Predict ratings for each candidate ─────────────────────────────────
    if user_id not in users["user_id"].values:
        return jsonify({"error": "User demographics not found"}), 400

    predictions = []
    for mid in list(candidate_ids)[:300]:   # cap for performance
        try:
    ...
    # Sort by predicted rating desc, then popularity
...
def analytics():
    ...
    top_movies["avg"] = top_movies["avg"].round(2)
    ...
    return jsonify({
        ...
        "top_movie_avgs":   top_movies["avg"].tolist(),
